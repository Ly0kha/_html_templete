...
// btn //
function writeBtnDocument(rootDir) {

	$.ajax({
		url: rootDir + "include/btn.html", // パスはplugin.jsが読み込まれたHTMLファイルが基準 //
		cache: false, // キャッシュを利用 //
		async: false, // 非同期で読み込む //
		success: function(html){
			html = html.replace(/\{\$root\}/g, rootDir);
...
if (ua.indexOf('iPhone') > 0 || ua.indexOf('iPad') > 0 || ua.indexOf('iPod') > 0 || ua.indexOf('Android') > 0 || ua.indexOf('BlackBerry') > 0 || ua.indexOf('windows Phone') > 0 || ua.indexOf('NOKIA') > 0 || /Mobile.*Firefox/.test(ua)) {
	...
	$(function() {
		...
		var prop = $("<div id='big-anime-cover'></div><div id='big-anime-wrap'></div>");
		$("body#top").after(prop);

		// var BV = new $.BigVideo();
		// BV.init();
		// BV.show('assets/christmas_snow.swf',{doLoop:true});

		// var BV = new $.BigVideo();
		// BV.init();
		// BV.show('assets/christmas_snow_lite-compressor.gif',{doLoop:true});
	...
	$(function() {
		...
		$("body#top").after("<div id='big-anime-cover'></div>");

		var BV = new $.BigVideo();
		BV.init();
		BV.show('assets/christmas_snow.mp4',{doLoop:true});
